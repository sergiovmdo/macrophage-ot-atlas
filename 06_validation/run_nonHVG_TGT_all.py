#!/usr/bin/env python
"""Non-HVG TGT gradient analysis — ALL filtered genes, vectorized."""
...
for key, src_g in TRANSITIONS:
    ...
    # Precompute quartile masks once per transition
    q_masks = {q: quartiles == q for q in ["Q1", "Q2", "Q3", "Q4"]}
    q_counts = {q: m.sum() for q, m in q_masks.items()}

    tgt_file = DEG_DIR / f"{key}_tgt_up_nonHVG_DEGs.csv"
    if not tgt_file.exists():
        print(f"  No TGT DEG file")
        continue

    tgt_degs = pd.read_csv(tgt_file)
    tgt_named = tgt_degs[~tgt_degs["names"].str.startswith(EXCLUDE_PREFIXES)]
    tgt_filt = tgt_named[tgt_named["scores"] > 5].copy()
    genes = [g for g in tgt_filt["names"].tolist()
             if g in adata.var_names and g in safe_set]

    print(f"  TGT genes to test: {len(genes)}")
    if not genes:
        continue
    ...
    for chunk_start in range(0, len(genes), CHUNK):
        ...
        for gi, gene in enumerate(chunk_genes):
            if gene not in col_names:
                continue
            col_idx = col_names.index(gene)
            e = X[:, col_idx]

            # Observed quartile means
            ...
            all_results.append({
                "transition": key, "gene": gene, "role": "TGT",
...
results_df.to_csv(OUT / "nonHVG_TGT_allgenes_results.csv", index=False)

# Summary
print(f"\n{'='*60}")
print("SUMMARY — TGT GENES ONLY (all filtered, no cap)")
print(f"{'='*60}")
print(f"{'Transition':20s}  {'Tested':>6}  {'Sig':>5}  {'Up':>4}  {'Sig%':>6}")
print("-" * 50)

for key, grp in results_df.groupby("transition"):
    n = len(grp)
    n_sig = (grp["perm_p"] <= 0.001).sum()
    n_up = len(grp[(grp["perm_p"] <= 0.001) & (grp["log2FC"] > 0)])
    print(f"{key:20s}  {n:6d}  {n_sig:5d}  {n_up:4d}  {100*n_sig/n:.0f}%")

total = len(results_df)
total_sig = (results_df["perm_p"] <= 0.001).sum()
total_up = len(results_df[(results_df["perm_p"] <= 0.001) & (results_df["log2FC"] > 0)])
expected = total * 0.001
print(f"\n{'OVERALL':20s}  {total:6d}  {total_sig:5d}  {total_up:4d}  {100*total_sig/total:.0f}%")
print(f"Expected by chance: {expected:.1f}")
print(f"Enrichment: {total_sig/expected:.0f}-fold")
print(f"\nTotal runtime: {time.time()-t0:.0f}s")
print(f"Saved to {OUT / 'nonHVG_TGT_allgenes_results.csv'}")